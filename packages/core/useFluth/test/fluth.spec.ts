...
describe.sequential("useFluth", () => {
  ...
  it("multiple streams should work with complex computed using native reactivity", async () => {
    ...
    expect(wrapper.find('[data-testid="quantity"]').text()).toBe("Quantity: 2");
    ...
    expect(wrapper.find('[data-testid="quantity"]').text()).toBe("Quantity: 3");
    expect(wrapper.find('[data-testid="subtotal"]').text()).toBe(
      "Subtotal: 450",
    );
  ...
  it("stream with object properties should trigger reactivity on property access", async () => {
    ...
    const userStream$ = $({ name: "John", age: 25, email: "john@example.com" });
    ...
    // Initial state
    expect(wrapper.find('[data-testid="user-name"]').text()).toBe("Name: John");
    ...
    expect(wrapper.find('[data-testid="user-name"]').text()).toBe("Name: Bob");
