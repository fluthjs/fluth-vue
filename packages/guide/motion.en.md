...
::: details Differences between fluth streams and promises:
...
::: details Differences between fluth streams and RxJS:
...
fluth saves the data processed by logic for each stream node, allowing stream nodes to carry both logic and data, as shown below:
...
For large-scale use of streams, fluth-vue has enhanced fluth streams:
...
- For fluth-vue, stream data is read-only reactive data that can be used normally in templates, watch, computed, and can also use the [toCompt](/en/useFluth/#tocompt) method to convert to computed reactive data, so frameworks can directly consume stream data and view stream data directly through vue-devtools

### Debugging Capabilities

fluth uses immutable data structures at the bottom layer and provides rich debugging plugins:
...
- **Expressiveness improvement**: Operators like [audit](https://fluthjs.github.io/fluth-doc/en/api/operator/audit.html), [debounce](https://fluthjs.github.io/fluth-doc/en/api/operator/debounce.html), [filter](https://fluthjs.github.io/fluth-doc/en/api/operator/filter.html) handle complex asynchronous control logic such as triggers, throttling, conditional filtering in a declarative manner. Through stream operators, code expressiveness is significantly improved.
...
fluth-vue truly brings the stream programming paradigm to Vue developers: making streams the most basic form of data in frontend and perfectly compatible with reactivity, thoroughly implementing reactivity: not just data and view reactivity, but also organizing logic with stream reactivity.
